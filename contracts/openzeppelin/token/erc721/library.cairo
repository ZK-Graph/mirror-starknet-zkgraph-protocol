...
@storage_var
func ERC721_timestamp(token_id: Uint256) -> (timestamp: felt) {
}
...
namespace ERC721 {
    ...
    func timestamp_of{
        ...
        let timestamp : felt = ERC721_timestamp.read(token_id);
        with_attr error_message("ERC721: timestamp query for nonexistent token") {
    ...
    func get_approved{
        ...
        let exists : felt = _exists(token_id);
        with_attr error_message("ERC721: approved query for nonexistent token") {
            assert exists = TRUE;
        }

        let approved : felt = ERC721_token_approvals.read(token_id);
        return approved;
    }
    ...
    func is_approved_for_all{
            ...
            pedersen_ptr: HashBuiltin*,
            range_check_ptr
        }(owner: felt, operator: felt) -> felt {
        let is_approved : felt = ERC721_operator_approvals.read(owner=owner, operator=operator);
        return is_approved;
    }
    ...
    func token_uri{
        ...
        let token_uri : felt = ERC721_token_uri.read(token_id);
        return (token_uri);
    }
    ...
    func approve{
        ...
        if (caller == owner) {
            ...
            let is_approved : felt = ERC721_operator_approvals.read(owner, caller);
    ...
    func transfer_from{
        ...
        let caller : felt = get_caller_address();
        let is_approved : felt = _is_approved_or_owner(caller, token_id);
    ...
    func assert_only_token_owner{
        ...
        uint256_check(token_id);
        let caller : felt = get_caller_address();
        let owner : felt = owner_of(token_id);
    ...
    func _is_approved_or_owner{
        ...
        let approved_addr : felt = get_approved(token_id);
        if (approved_addr == spender) {
            return TRUE;
        }

        let is_operator : felt = is_approved_for_all(owner, spender);
        if (is_operator == TRUE) {
            return TRUE;
        }
    ...
    func _exists{
        ...
        let exists : felt = ERC721_owners.read(token_id);
    ...
    func _safe_transfer{
        ...
        let success : felt = _check_onERC721Received(from_, to, token_id, data_len, data);
    ...
    func _mint{
        ...
        ERC721_balances.write(to, new_balance);
        ERC721_owners.write(token_id, to);
        ERC721_timestamp.write(token_id, timestamp);
    ...
    func _safe_mint{
        ...
        let success : felt = _check_onERC721Received(
            0,
            to,
            token_id,
            data_len,
...
func _check_onERC721Received{
    ...
    let caller : felt = get_caller_address();
    let is_supported : felt = IERC165.supportsInterface(to, IERC721_RECEIVER_ID);
    if (is_supported == TRUE) {
        let selector : felt = IERC721Receiver.onERC721Received(
            to,
            caller,
            from_,
            token_id,
    ...
    let is_account : felt = IERC165.supportsInterface(to, IACCOUNT_ID);
    return is_account;
}
