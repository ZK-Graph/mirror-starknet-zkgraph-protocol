...
//    range_check_ptr
//    }(profile_id : Uint256) -> DataTypes.ProfileStruct {
//
...
@view func get_profile_by_id{
    ...
    pedersen_ptr : HashBuiltin*,
    range_check_ptr
    }(profile_id : Uint256) -> (pub_count : Uint256, follow_module : felt, follow_nft : felt, handle : felt, image_uri : felt, follow_nft_uri : felt) {
    let profile : DataTypes.ProfileStruct = profile_by_id.read(profile_id);
    return (profile.pub_count, profile.follow_module, profile.follow_nft, profile.handle, profile.image_uri, profile.follow_nft_uri);
}
...
func get_profile_element_by_id{
    ...
    pedersen_ptr : HashBuiltin*,
    range_check_ptr
    }(profile_id : Uint256, element : felt) -> felt {
    let profile : DataTypes.ProfileStruct = profile_by_id.read(profile_id);
...
namespace PublishingLogic {
    ...
    func create_profile{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, bitwise_ptr : BitwiseBuiltin*, range_check_ptr
        ...
        let handle_hash : Uint256 = get_keccak_hash(vars.handle);
        let handle_hash_felt : felt = uint256_to_address_felt(handle_hash);
        ...
        let struct_array : DataTypes.ProfileStruct* = alloc();
    ...
    func create_post{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr, bitwise_ptr : BitwiseBuiltin*
        ...
        alloc_locals;
        let struct_array : DataTypes.PublicationStruct* = alloc();
...
func _emit_profile_created{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr
    ...
    let sender_address : felt = get_caller_address();
    let block_timestamp : felt = get_block_timestamp();
